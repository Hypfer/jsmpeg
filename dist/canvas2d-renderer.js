...
export class Canvas2DRenderer {
    ...
    renderProgress(progress) {
        const w = this.canvas.width, h = this.canvas.height, ctx = this.context;
        ctx.fillStyle = '#222';
    ...
    YCbCrToRGBA(y, cb, cr, rgba) {
        ...
        // buffer.
        const w = ((this.width + 15) >> 4) << 4, w2 = w >> 1;
        let yIndex1 = 0, yIndex2 = w;
        const yNext2Lines = w + (w - this.width);
        let cIndex = 0;
        const cNextLine = w2 - (this.width >> 1);
        let rgbaIndex1 = 0, rgbaIndex2 = this.width * 4;
        const rgbaNext2Lines = this.width * 4;
        const cols = this.width >> 1, rows = this.height >> 1;
        let ccb, ccr, r, g, b;
