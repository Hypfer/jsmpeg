// Hardware YCbCr -> RGBA renderer. Uploads the 3 planes as textures and does
// the colour conversion on the GPU in a fragment shader. Default renderer.
export class WebGLRenderer {
    constructor(options) {
        this.enabled = true;
        this.hasTextureData = {};
        this.shouldCreateUnclampedViews = false;
        if (options.canvas) {
        ...
        const gl = this.canvas.getContext('webgl', contextCreateOptions) ||
    ...
    compileShader(type, source) {
        ...
        if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
            throw new Error((_a = gl.getShaderInfoLog(shader)) !== null && _a !== void 0 ? _a : 'Shader compile error');
        }
        return shader;
    }
    ...
    render(y, cb, cr, isClampedArray) {
        ...
        const gl = this.gl;
        const w = ((this.width + 15) >> 4) << 4, h = this.height, w2 = w >> 1, h2 = h >> 1;
